...
create view osbiz_pruefungen as
	...
	from mvhs.v
	where (
		(v.kst_kennz='7200' and
                 substr(v.vnr,7,1)='A' and
		 instr(upper(v.utext1),'ORIENTIERUNGSKURS')>0
                )
                or
                (v.kst_kennz='7250')
	      )
...
select 
	...
	s.AP_INFO,
	s.RUECKGABE_DATUM
from MVHS.T t, KLARO.SCHUELER s
where t.TNR = s.ID
and s.ID in (select id from osbiz_schueler)
ORDER BY 3, 4;
...
begin
	...
	set NACHNAME = :new.NACHNAME, 
		...
		BEMERKUNG = :new.BEMERKUNG,
		AENDERUNG_VON = :new.AENDERUNG_VON,
		WDH_BAMF_NUMMER = :new.WDH_BAMF_NUMMER,
		AP_NAME = :new.AP_NAME,
...
where t.TNR = l.ID
  and l.ID in (select id from osbiz_lehrer)
ORDER BY 2, 3;
